...
[GitHub releases page](https://github.com/NitzanLux/mnelab-streams/releases); this page is generated from
[`CHANGELOG.md`](https://github.com/NitzanLux/mnelab-streams/blob/main/CHANGELOG.md) by `tools/changelog.py docs`.
...
- Select filter target streams and channels before configuration, add band-stop filtering, and expose EDFbrowser-inspired Butterworth, Chebyshev, Bessel, moving-average, order, ripple, resonator Q-factor, bandwidth, and notch-harmonic controls (by [NitzanLux](https://github.com/NitzanLux))
...
- Allow each stream-viewer channel to use its own display unit, including custom IMU and sensor unit labels, with persistence in display montages
- Add right-click stream display properties with exact per-stream physical amplitude scales and reversible lane fitting
- Add reusable JSON filter presets for per-stream filter operations and channel targets
- Add live per-stream filter-response plots and an Apply & Add Another workflow for ordered filter stages
...
- Compact the annotation sidebar filter panel into a grid with inline placeholders, icon clear buttons, and paired checkboxes (by [NitzanLux](https://github.com/NitzanLux))
...
These versions were released by upstream [MNELAB](https://github.com/cbrnr/mnelab) before this fork started its own release series. No MNELAB Streams builds exist for them.

??? note "1.5.6 · 2026-07-06"
    ### 🔧 Fixed
