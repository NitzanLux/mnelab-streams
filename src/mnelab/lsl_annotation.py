...
# remains readable in the timeline even when the pinned guide is unavailable.
DISPLAY_WIDTH = 80
...
def _field_lines(label, fields):
    """Render a mapping as wrapped `label: key=value` text."""
    body = ", ".join(
...
def _fallback_format_marker(marker, *, include_uids=False):
    """Return a compact marker label when the guide checkout is unavailable."""
    path = "/".join(
...
def format_marker(marker, *, include_uids=False):
    ...
    The guide owns this presentation when its checkout is available. A compact
    fallback keeps valid annotations readable in installed builds that do not
    include that checkout.
    """
