...
Existing functionality is part of the product contract. Do not remove, disable,
silently alter, or make inaccessible any existing functionality unless the task
explicitly authorizes that specific change.

The current behavioral contract is defined by:

- `docs/software-specification.md` for intended user-visible behavior
- automated tests for executable behavioral guarantees
- `FORK_CHANGES.md` for behavior introduced by this fork
- `CHANGELOG.md` for historical changes

Before making a non-trivial change:

1. Read the relevant sections of the software specification and existing tests.
2. Identify the existing behaviors and workflows affected by the change.
3. Preserve those behaviors unless their modification is explicitly requested.
4. Add or update tests for both the new behavior and preserved behavior.
5. Verify that the full test suite passes.

When modifying working but insufficiently tested code, add characterization tests
that capture its current observable behavior before refactoring it.

Existing functionality includes, but is not limited to:

- user-visible actions, menus, dialogs, and workflows
- supported file formats and import/export behavior
- public Python APIs and console commands
- saved settings and configuration compatibility
- signal-processing behavior and scientific semantics
- timestamp, acquisition-gap, and annotation handling
- behavior inherited from upstream MNELAB

If a requested change conflicts with existing functionality, or if it is unclear
whether a behavior may be changed, stop and describe the conflict before
implementing it. Do not resolve the conflict by silently removing the existing
behavior.

Bug fixes may intentionally change incorrect behavior, but the correction must be
explicitly described, tested, documented when user-visible, and recorded in the
changelog.
...
- previously supported functionality remains available unless its change or
  removal was explicitly requested;
- `docs/software-specification.md` is updated when the product contract changes;
- user-visible changes are documented;
- `CHANGELOG.md` contains an appropriate `[UNRELEASED]` entry;
- the final diff contains no unrelated changes;
- the final response lists what was changed and how it was verified.
